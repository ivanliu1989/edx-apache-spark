...
aSparse = SparseVector(len(aDense), [0,1,2,3], aDense)

bDense = np.array([0., 0., 0., 1.])
bSparse = SparseVector(len(bDense), [0,1,2,3], bDense)
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    return SparseVector(numOHEFeats, sorted([OHEDict[x] for x in sorted(rawFeats)]), np.ones(len(rawFeats)))
...
def parsePoint(point):
    ...
    """
    point_split = point.split(",")[1:]
    return [(x, point_split[x]) for x in range(0,(len(point_split)))]
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    var = [(x, point_split[x]) for x in range(0,(len(point_split)))]
    return LabeledPoint(target, oneHotEncoding(var,OHEDict,numOHEFeats))
