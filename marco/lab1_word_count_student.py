...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda (v,s): (v, sum(s)))
...
def wordCount(wordListRDD):
    ...
    """
    wordRDD = wordListRDD.map(lambda x: (x,1)).reduceByKey(lambda x,y: x+y)
    return wordRDD
...
def removePunctuation(text):
    ...
    """
    ctext = re.sub(r'[^\w\s]','',text).lower().strip()
    return ctext
